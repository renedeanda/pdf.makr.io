  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="relative w-full max-w-2xl max-h-[80vh] overflow-auto bg-white dark:bg-surface-100 rounded-2xl shadow-2xl animate-in zoom-in-95 duration-200">
          {/* Header */}
          <div className="sticky top-0 bg-white dark:bg-surface-100 border-b border-border-light p-6 flex items-center justify-between z-10">
            ...
            <button
              onClick={() => setIsOpen(false)}
              className="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-surface-100 dark:hover:bg-surface-200 text-text-secondary hover:text-text-primary transition-colors"
              aria-label="Close"
            >
              <X className="h-5 w-5" />
            </button>
                ...
                <div className="space-y-2">
                  {shortcuts
                    ...
                    .map((shortcut, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between gap-4 p-3 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-200 transition-colors"
                      >
                        ...
                        <div className="flex gap-1">
                          {shortcut.keys.map((key, keyIndex) => (
                            <kbd
                              key={keyIndex}
                              className="px-2 py-1 text-xs font-semibold text-text-primary bg-surface-100 dark:bg-surface-200 border border-border-medium rounded shadow-sm min-w-[2rem] text-center"
                            >
                              {formatKey(key)}
                            </kbd>
                          ))}
          ...
          {/* Footer */}
          <div className="sticky bottom-0 bg-surface-50 dark:bg-surface-200 border-t border-border-light p-4 text-center">
            <p className="text-xs text-text-tertiary">
              Press <kbd className="px-1.5 py-0.5 text-xs font-semibold text-text-primary bg-surface-100 dark:bg-surface-100 border border-border-medium rounded">?</kbd> to toggle this dialog
            </p>
          </div>
        </div>
      </div>
